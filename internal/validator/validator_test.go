...
func (s *ValidatorSuite) TestItEventuallyBecomesReady() {
	validator := NewValidator(s.repo, uint64(10), uint64(0))
...
func (s *ValidatorSuite) TestItFinishesAnEmptyEpoch() {
	...
	inputBoxDeploymentBlock := uint64(0)
	currentEpoch := Epoch{inputBoxDeploymentBlock, inputBoxDeploymentBlock + epochDuration}
	expectedEpoch := Epoch{currentEpoch.EndBlock + 1, currentEpoch.EndBlock + epochDuration}
...
func (s *ValidatorSuite) TestItFinishesAnEpochWithOutputs() {
	...
	s.repo.On(
		...
		mock.Anything,
	).Return([]Output{{uint64(0), uint64(0), []byte{}}}, nil)
	...
	for {
		...
		case err := <-errChannel:
			...
			expectedClaim := &Claim{
				uint64(0),
				uint64(0),
				crypto.Keccak256(genericData, genericData),
			}
			expectedProofs := []Proof{{}}
			s.repo.AssertCalled(
				s.T(),
...
func (m *MockRepository) FinishEpochTransaction(
	...
	args := m.Called(ctx, newEpoch, claim, proofs)
	return args.Error(0)
}

...
func (suite *LoadSuite) SetupTest() {
	...
	suite.Nil(err)
	suite.config = &emulator.MachineRuntimeConfig{}
}
...
func (suite *LoadSuite) TestInvalidAddress() {
	...
	suite.NotNil(err)
	suite.Nil(m)
	suite.Equal(emulator.ErrCodeRuntimeError, err.(*emulator.Error).Code)
}
...
func (suite *AdvanceInspectSuite) SetupTest() {
	...
	machine, err := machine.Load(address,
		"advance-inspect/snapshot",
...
func (suite *AdvanceInspectSuite) TestSingleOutput() {
	...
	outputs, _, err := suite.machine.Advance(request)
	suite.Nil(err)
	suite.Len(outputs, 1)
	// suite.Something(hash, something)
